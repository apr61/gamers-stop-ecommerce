...
function AddressFormPage() {
  ...
  function handleSubmit(e) {
    ...
    try {
      setLoading(true);
      if (edit) {
        updateAddress(newAddress, preAddress?.id).then(
          updateLocalAddress({ id: preAddress?.id, ...newAddress })
        );
      } else {
        addAddress(newAddress).then((docRef) =>
  ...
  return (
    <section className="address-form-sec main">
      ...
      <form className="address-form" onSubmit={handleSubmit}>
        ...
        />
        <Input
          labelName="Flat, House no, Building, Apartment"
          ref={flatRef}
          value={preAddress?.flat}
        />
        <Input
          labelName="Area, Street, Village"
          ref={areaRef}
          value={preAddress?.area}
        />
        <Input
          labelName="Landmark"
          placeholder="eg. near Med Plus Store"
          ref={landmarkRef}
